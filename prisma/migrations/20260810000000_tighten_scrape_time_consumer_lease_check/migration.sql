...
ALTER TABLE "BankSessionExpiryEpisode"
  DROP CONSTRAINT "BankSessionExpiryEpisode_consumerLease_check",
  ADD CONSTRAINT "BankSessionExpiryEpisode_consumerLease_check" CHECK (
    ...
    OR (
      ...
      AND (
        ("consumerAttemptState" IN ('reserved', 'mutation_started') AND "consumerLeaseExpiresAt" IS NOT NULL AND "terminalFailureReason" IS NULL)
        OR ("consumerAttemptState" IN ('manual_recovery_required', 'resolved') AND "consumerLeaseExpiresAt" IS NULL)
      )
      AND (
        ("consumerAttemptSource" = 'scheduled' AND "publicationState" = 'published' AND "publicationClaimToken" ~ '[^[:space:]]')
        OR ("consumerAttemptSource" = 'scrape_time' AND "publicationState" = 'pending' AND "publicationClaimToken" IS NULL)
      )
    )
  );
