...
- [ ] B2.3 Consumer revalidation and attempt policy: each job carries `bankCode + expiredEventId + token + runId`; the consumer rereads PostgreSQL, treats the queue as a hint, and defines durable post-claim throttled/crash/manual behavior.
  ...
  - [ ] PR4p2a2: integrate the repository primitive into consumer acquisition and classify stale-versus-duplicate deliveries before any credential mutation.
  - [ ] PR4p2b+: define post-claim uncertainty/manual recovery, operator outcome/audit/replay, terminal-failure reconciliation, and observe-rejection signals.
