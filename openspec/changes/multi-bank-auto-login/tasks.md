...
- [ ] B2.3 Consumer revalidation and attempt policy: each job carries `bankCode + expiredEventId + token + runId`; the consumer rereads PostgreSQL, treats the queue as a hint, and defines durable post-claim throttled/crash/manual behavior.
  ...
  - [ ] PR4p2: durable claim/no-duplicate-submission before any credential mutation; post-claim uncertain/manual recovery, operator outcome/audit/replay, terminal-failure reconciliation, observe-rejection signal, and PostgreSQL interleavings.
