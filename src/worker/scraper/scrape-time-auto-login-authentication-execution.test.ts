...
import { executeScrapeTimeAutoLoginAuthenticationAttempt, type BankAutoLoginOutcome } from "./auto-login";
...
describe("createScrapeTimeAutoLoginAuthenticationExecution", () => {
  it("validates exact job data without invoking getters or retaining caller data", () => {
    ...
    const cases = [null, 1, {}, { data: { bankId: "popular", runId: "run" } }, { data: { ...job.data, expiredEventId: undefined } }, { data: { ...job.data, extra: true } }, { data: { ...job.data, bankId: " " } }, { data: { ...job.data, runId: 1 } }, getter, { data: { ...job.data, [Symbol("x")]: true } }];
    for (const malformed of cases) expect(() => createScrapeTimeAutoLoginAuthenticationExecution({ runnerDependencies: base(), job: malformed as never, identity })).toThrow("Invalid authentication execution input");
    expect(reads).toBe(0);
  });
  ...
  it("blocks malformed outcomes, forged fences, pre-abort, and adapter reuse without dependencies", async () => {
    for (const outcome of [null, 1, { status: "succeeded", safeSummary: "secret" }, { status: "manual_required", reason: "wrong", safeSummary: "secret" }, { status: "needs_admin_action", reason: "wrong", safeSummary: "secret" }, { status: "throttled", safeSummary: "" }]) await expect(execute(outcome)).resolves.toEqual({ status: "blocked" });
  ...
  it("claims a real active fence once before bridge dependencies", async () => {
    const dependencies = base(); const adapter = createScrapeTimeAutoLoginAuthenticationExecution({ runnerDependencies: dependencies, job, identity }); const results: AuthenticationExecutionResult[] = []; const calls = () => (dependencies.adapterRegistry.get as unknown as { mock: { calls: unknown[] } }).mock.calls.length;
    await createAuthenticatedSessionMutationRunner({ execution: { execute: async (input) => { results.push(await adapter.execute(input)); const beforeDuplicate = calls(); const duplicate = await executeScrapeTimeAutoLoginAuthenticationAttempt({ runnerDependencies: dependencies, job, trigger: { kind: "authentication_attempt", id: "a".repeat(64) }, fence: input.fence, signal: input.signal }); expect([duplicate, calls()]).toEqual([{ outcome: null, durableResult: { status: "blocked" } }, beforeDuplicate]); return results[0]!; } }, heartbeat: { start: () => ({ stop: async () => undefined }) } }).run(await authority());
    expect(results).toEqual([{ status: "succeeded" }]);
  });
});
